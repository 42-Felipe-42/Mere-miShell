...
/*   Updated: 2024/07/31 18:53:32 by louismdv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	go_home(t_shell *shell, char *old_pwd)
{
	...
	if (!home_dir)
		return (ft_putstr_fd("cd: HOME not set\n", STDERR_FILENO),
			shell->exit_code = 1, 0);
	if (chdir(home_dir) == -1)
		return (print_error(home_dir), free(old_pwd), shell->exit_code = 1, 0);
...
void	update_env_vars(t_shell *shell, char *oldpwd, char *currpwd)
{
	...
	while (env_entry)
	{
				...
				ft_strlen(env_entry->key)) == 0)
		{
			...
			if (!env_entry->value)
				(free(oldpwd), exit_and_free(shell, " : malloc failed"));
		}
		if (ft_strncmp(env_entry->key, "PWD", ft_strlen(env_entry->key)) == 0)
		{
			...
			if (!env_entry->value)
				(free(oldpwd), exit_and_free(shell, "Error : malloc failed"));
		}
		env_entry = env_entry->next;
	}
}
