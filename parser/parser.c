...
/*   Updated: 2024/07/03 16:44:50 by lmerveil         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	is_builtin(t_cmds *cmds, t_lexer *lex)
{
	if (lex->word != NULL)
	{
		if (ft_strncmp(lex->word, "echo", 4) == 0 && ft_strlen(lex->word) == 4)
			...
			cmds->builtin = PWD;
		else if (ft_strcmp(lex->word, "export") == 0
			&& ft_strlen(lex->word) == 6)
			cmds->builtin = EXPORT;
		else if (ft_strcmp(lex->word, "unset") == 0
			&& ft_strlen(lex->word) == 5)
			cmds->builtin = UNSET;
		else if (ft_strcmp(lex->word, "env") == 0 && ft_strlen(lex->word) == 3)
			cmds->builtin = ENV;
		else if (ft_strncmp(lex->word, "exit", 4) == 0
			&& ft_strlen(lex->word) == 4)
			cmds->builtin = EXIT;
	}
}
